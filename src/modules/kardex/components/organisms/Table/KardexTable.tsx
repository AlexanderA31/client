...
export function KardexTable({ kardexData, loading, viewType, onViewDetails }: Props) {
	...
	const columns = createTableColumns({ onViewDetails })
		...
		<AnimatePresence mode='popLayout' initial={false}>
			<motion.div
				...
				<motion.div variants={animations.container} className='h-full' layout>
					...
					{viewType === 'card' && <CardView table={table} onViewDetails={onViewDetails} />}
					{viewType === 'list' && <ListView table={table} onViewDetails={onViewDetails} />}
				</motion.div>
			</motion.div>
		</AnimatePresence>
	)
